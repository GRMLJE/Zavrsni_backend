...
def _build_events_query(viewer_user_id, city_id, neighborhood_id, category_id, user_id):
    query = """
        SELECT
...
def get_event(event_id, viewer_user_id=None):
    ...
    if not row:
        return None
    d = _row_to_dict(row)
    d["organizer_name"] = row[11]
    d["organizer_email"] = row[12]
    return d
...
def join_event(event_id, user_id):
    with get_db() as conn:
        with conn.cursor() as cur:
            cur.execute("SELECT id, user_id FROM events WHERE id = %s;", (event_id,))
            event = cur.fetchone()
            if not event:
